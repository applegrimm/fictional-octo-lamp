...
> ## 方針（決定済み）
>
> **現行システムは作り直す。完全再現はしない。**
>
> この文書は「移行元の調査結果」であり、「新システムの仕様書」ではない。
> 🐛 印のバグ、および「未使用」と記した機能は**移植しない**。
> 新システムの設計は `docs/MIGRATION.md` を参照。
...
- 受取希望日は**商品を1つ以上選択するまで入力できない**（`disabled`）
  - 🐛 **UXの問題（確認済み）**: 商品を選んでいても**数量が空またはゼロ**だと
    同じく無効のままになる。日付欄が灰色のまま反応せず、時間のプルダウンも空。
    それでいて両方とも必須項目なので送信できず、利用者は理由が分からず詰まる。
    唯一の手がかりは日付欄の下に出る「商品を選択してください。」という文言だが、
    商品は選択済みなので誤解を招く。
    **新システムでは、数量未入力を明示するか、数量の初期値を1にすること。**
...
> 🐛 **バグ（確認済み）**: `minType === 'business'` の場合、`cutoff` が設定されていても
> 締切時刻が無視される。暦日指定の商品だけ締切が効く。
> 実際に `p1`（`minType: business`, `cutoff: "15:00"`）は締切が機能していない。
> **意図的な仕様ではないことを確認済み。新システムでは営業日指定でも締切を効かせる。**
