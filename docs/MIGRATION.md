...
|---|---|---|
| A | 稼働開始 | **2026年11月末** |
| B | 店舗管理画面の認証 | **シークレットURL継続**。ただしトークンはDBで管理し、個別に失効できるようにする |
| C | Excel出力の列構成 | 未定（現行の受注集計ファイルを見てから決める） |
| D | 旧データの移行 | **移行しない。** スプレッドシートは参照用に保管 |
| E | 独自ドメイン | 未定（Stripe のリダイレクトURL設定に影響。切り替え前に決める） |

---

## 8. スケジュール

稼働目標 **11月末**。年末商戦の受注開始に間に合わせ、12月からは新システムで受ける。

| 時期 | フェーズ | 内容 |
|---|---|---|
| 8月下旬 | 1 | 環境構築・D1スキーマ・マスタ投入 |
| 9月上旬 | 2 | **受取日/配送日の算出ロジック + テスト** ← 最重要 |
| 9月中旬〜下旬 | 3 | 予約フォーム（3入口）+ 注文作成API |
| 10月上旬 | 4-5 | Stripe Checkout + Webhook、法人請求書 |
| 10月中旬 | 6 | メール（Resend） |
| 10月下旬 | 7-8 | 店舗管理画面、マスタ管理画面、Excel出力 |
| 11月 | 9 | 並行稼働・店舗への周知・切り替え |
| 12月〜1月 | — | 本番運用（年末年始ピーク） |

**11月を丸ごと検証期間に充てられる。** ここが今回の時期設定の狙い。
フェーズ2を9月上旬に固めておけば、以降で壊れてもテストで検出できる。

### 繁忙期前に必ずやること

- Resend を有料プラン（$20/月・50,000通）に切り替え。無料枠は 100通/日 のため、
  **50注文/日を超えると現行と同じ問題が起きる**
- Stripe を LIVE モードに切り替え、少額の実決済で確認
- 店舗管理URLを24店舗へ配布し、各店舗で開けることを確認
...
| # | 論点 | いつまでに |
|---|---|---|
| C | Excel出力の列構成 | フェーズ8着手前。現行の受注集計ファイルを共有してもらう |
| E | 独自ドメイン | フェーズ4着手前。Stripe のリダイレクトURLに影響 |
